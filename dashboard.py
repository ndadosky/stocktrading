...
from scanner_config import (
    DASHBOARD_FILE, PAPER_TRADES_FILE, STARTING_CAPITAL, WATCHLIST_EXPORT_DIR,
    ensure_directories,
)
...
def trade_table(frame: pd.DataFrame) -> str:
    ...
    columns = ["trade_date", "ticker", "sector", "entry_price", "current_price", "shares", "remaining_shares",
               "p_l", "p_l_%", "status", "exit_reason", "holding_days", "confirmation_band",
               "shares_sold_10", "shares_sold_20", "shares_sold_30", "market_regime", "spread_proxy_pct"]
...
.strategy{{background:linear-gradient(145deg,#f8fbff,#eff6ff);border-color:#dbeafe}}.strategy p{{color:#4b5563;line-height:1.6;margin:10px 0 20px}}.schedule{{display:grid;gap:0}}.schedule div{{display:flex;justify-content:space-between;padding:11px 0;border-top:1px solid #dbeafe}}.schedule b{{font-size:12px}}.schedule span{{color:var(--muted);font-size:12px}}
...
<aside class='panel strategy'><h2>Automated controls</h2><p>Scale out 50 shares at +10%, 25 at +20%, then take the final 25 at +30% or protect them at +10%.</p><div class='schedule'><div><b>Exit ladder</b><span>50 / 25 / 25</span></div><div><b>Protective stop</b><span>−8%</span></div><div><b>Time stop</b><span>10 sessions</span></div><div><b>Fill slippage</b><span>10 bps</span></div><div><b>Sector ceiling</b><span>25%</span></div></div></aside>
