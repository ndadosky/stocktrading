...
@media(max-width:1100px) and (max-height:650px){
  ...
  .home-grid{display:grid;grid-template-columns:none;grid-auto-flow:column;grid-auto-columns:calc(100vw - 28px);gap:10px;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x mandatory;scroll-padding-inline:2px;overscroll-behavior-x:contain;touch-action:pan-y pinch-zoom;user-select:none;-webkit-user-select:none;scrollbar-width:none;padding:1px 2px 6px;cursor:grab}.home-grid.dragging{cursor:grabbing;scroll-snap-type:none}.home-grid::-webkit-scrollbar{display:none}.home-card,.cars-card,.ewelink-card,.solar{grid-column:auto;scroll-snap-align:center;scroll-snap-stop:always;min-height:304px;padding:16px 18px;border-radius:14px}.card-head{margin-bottom:14px}.hero-value{font-size:40px}.detail-row{margin-top:13px}.control-btn{min-height:46px;padding:10px 15px}.control-row>.control-btn{width:auto;max-width:none;flex:1}.stepper .control-btn{width:46px}.solar-layout{grid-template-columns:minmax(190px,.72fr) minmax(0,1.28fr);gap:22px}.solar-chart,.solar-chart-empty{height:122px}.cars-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.cars-card .card-head{margin-bottom:8px}.car{padding:10px 12px}.car-image{height:105px}.car-metrics{margin:0 0 8px}.car-metric strong{font-size:21px}.ewelink-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
}
